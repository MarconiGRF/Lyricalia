...
struct SpotifyController: RouteCollection {
    ...
    func dispatchProcessUserLibrary(req: Request) async throws -> Bool {
        let givenUser = try req.content.decode(User.self)
        guard let userId = givenUser.id else {
            throw Abort(.badRequest, reason: "User ID is required to process the user's library")
        }
        guard let spotifyToken = givenUser.spotifyToken else {
            throw Abort(.badRequest, reason: "Spotify Token is required to process the user's library")
        }

        guard let user = try await User.find(givenUser.id, on: req.db) else {
            throw Abort(.badRequest, reason: "User ID is invalid, therefore cannot process their library")
        }
