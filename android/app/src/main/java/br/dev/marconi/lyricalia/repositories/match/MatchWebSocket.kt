...
class MatchWebSocket {
        ...
        try {
            webSocket = client.newWebSocket(request, object : WebSocketListener() {
                override fun onOpen(webSocket: WebSocket, response: Response) {
                    Log.d("IF1001_P3_LYRICALIA", "OPENED Websocket to Match")
            ...
            throw Exception("Could not connect to Match WebSocket", ex)
        }
    }
    ...
    fun send(message: String) {
        try {
            ...
            throw Exception("Could not send message to Match WebSocket", ex)
        }
    }
}